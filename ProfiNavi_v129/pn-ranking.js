(function(){
  ...
  function fromBundle(profile,services=[],works=[],slots=[]){
    ...
    const slotMap={},slotIntervals={},slotIsoMap={};slots.forEach(x=>{const d=new Date(x.starts_at),k=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`,t=`${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;(slotMap[k]||(slotMap[k]=[])).push(t);((slotIsoMap[k]||(slotIsoMap[k]={})))[t]=x.starts_at;const start=d.getTime(),end=start+scheduleStep*60000;(slotIntervals[k]||(slotIntervals[k]=[])).push({start,end,time:t,iso:x.starts_at})});

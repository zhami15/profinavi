...
function renderSnap(filter='all'){
 ...
 box.innerHTML=activeWorks.length?activeWorks.map((x,visibleIndex)=>{const m=masters[x.master];if(!m)return'';return `<button class="works-grid-item" onclick="openWorksViewer(${visibleIndex})" aria-label="${secondaryEsc(x.service||'Работа')}, ${secondaryEsc(m.name)}"><img src="${x.photo}" alt="Работа ${secondaryEsc(m.name)}" loading="lazy"></button>`}).join(''):`<div class="favorite-page-empty" style="grid-column:1/-1"><span>✦</span><h2>Пока нет работ</h2><p>Работы опубликованных мастеров появятся здесь.</p></div>`;
}
function openWorksViewer(index){
 ...
 list.innerHTML=activeWorks.map((x)=>{
  ...
  <img class="works-viewer-photo" src="${x.photo}" alt="Работа ${secondaryEsc(m.name)}">
  <div class="works-viewer-info">
   <button class="works-viewer-master" onclick="openProfile(${x.master})"><img src="${m.avatar}" alt="${secondaryEsc(m.name)}"><span><b>${secondaryEsc(m.name)}</b><small>${secondaryEsc(m.district||'')}</small></span></button>

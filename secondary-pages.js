...
function renderSnap(filter='all'){
 ...
 box.innerHTML=activeWorks.length?activeWorks.map((x,visibleIndex)=>{const m=masters[x.master];if(!m)return'';return `<button class="works-grid-item" onclick="openWorksViewer(${visibleIndex})" aria-label="${x.service||'Работа'}, ${m.name}"><img src="${x.photo}" alt="Работа ${m.name}" loading="lazy"></button>`}).join(''):`<div class="favorite-page-empty" style="grid-column:1/-1"><span>✦</span><h2>Пока нет работ</h2><p>Работы опубликованных мастеров появятся здесь.</p></div>`;
}
